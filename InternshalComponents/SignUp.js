...
const SignUp = (props) => {
    ...
    return (
        <div className={" s_col items-center md:w-4/12    h-full  rounded-xl"}>
            <h1 className={"font-bold text-xl lg:mt-4 md:mt-2"}>Sign Up</h1>
            <form action="" className={"flex-col md:w-3/4 sm:w-full mt-4"}>
               <label className={"block  w-11/12 sm:w-2/3 text-base lg:mt-1 md:mt-0"} htmlFor="name">Name</label>
               <input className={"block outline-none rounded-sm h-7  lg:mt-1 md:mt-0 bg-blue-100 w-full "} type="text" name="name" id="name" />
               ...
               <label className={"block  w-11/12 text-base lg:mt-1 md:mt-0 "} htmlFor="cpass">Confirm Password</label>
               <input className={"block outline-none rounded-sm h-7 lg:mt-1 md:mt-0 bg-blue-100 w-full "} t type="password" name="cpass" id="cpass" />
            <div className={"flex justify-center "}>

            <button type="submit" className={"w-4/6 h-7 md:w-5/6 lg:w-1/3 md:h-6 lg:h-7 bg-blue-600 lg:mt-5 md:mt-2 mt-3 rounded-md text-gray-50 "}>Sign Up</button>
            </div>
            </form>

<div className={"flex lg:mt-8 md:mt-3 w-full items-center mt-3  "}>
...
<div className={"flex lg:mt-8 md:mt-6 justify-around w-3/4 mt-3"}>
...
<div className={"lg:mt-12 md:mt-6 md:text-xs lg:text-sm flex justify-between w-full mt-5 text-xs"}>
    <p className>Already have an Account?</p>
    <p onClick={changePageHandler} className={"md:text-xs lg:text-sm text-xs md:text-black text-blue-500"}>Login </p>
